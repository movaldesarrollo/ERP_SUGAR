...
#End Region
    ...
    'Guarda o actualiza los vencimientos. UPDATE 05-05-2017
        ...
        If iidVencimiento = 0 Then
            ...
            For Each item In lvVencimientos.Items
                If item.SubItems(4).text = "PENDIENTE" Then
                    idItem = item.Text
                    dts.gidVencimiento = idItem
                    dts.gVencimiento = item.SubItems(1).text
                    dts.gImporte = FormatNumber(item.SubItems(2).text, 2)
                End If
            Next
            If idItem <> 0 Then
                dts.gCobrado = False
                dts.gDevuelto = False
                dts.gImporte = dts.gImporte + (totalFact - contarRestante())
                funcVE.actualizar(dts)
            Else
                dts.gCobrado = False
                dts.gDevuelto = False
                dts.gVencimiento = Date.Today
                dts.gImporte = totalFact - contarRestante()
                funcVE.insertar(dts)
            End If
            reload()
        Else
            Dim dImporte As Double = FormatNumber(Importe.Text, 2)
            Dim dImporteLinea As Double = FormatNumber(Replace(lvVencimientos.Items(indice).SubItems(3).Text, "€", ""), 2)
            If dImporte <= dImporteLinea Then
                ...
                End If
                funcVE.actualizar(dts)
                If lvVencimientos.Items(indice).SubItems(3).Text > dts.gImporte Then
                    dts.gImporte = lvVencimientos.Items(indice).SubItems(3).Text - Importe.Text
                    dts.gCobrado = False
                    dts.gDevuelto = False
                    dts.gVencimiento = Date.Today
                    funcVE.insertar(dts)
        ...
        If lvVencimientos.SelectedItems.Count > 0 Then
            ...
            Else
                If MsgBox("¿Está seguro que quiere borrar este vencimiento?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    If indice <> -1 Then
                        funcVE.borrar(lvVencimientos.Items(indice).SubItems(0).Text)
