Public Class GestionTiposRetencion
    Private iidTipoRetencion As Integer
    Private indice As Integer
    Private vSoloLectura As Boolean
    Private sDescripcion As String
    Private funcTE As New FuncionesTiposRetencion
    ...
    Private Sub borrarcampos()
        ...
        NombreTipoRetencion.Text = ""
        descripcion.Text = ""
        iidTipoRetencion = 0
        TipoRetencion.Text = 0
    ...
    Private Sub guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles guardar.Click
        ...
        Try
            ...
            If Repetido() Then
                validar = False
                ep1.SetError(NombreTipoRetencion, "Ya existe el tipo de IVA")
            End If
            If validar Then
                Dim dts As New DatosTipoRetencion
                dts.gidTipoRetencion = iidTipoRetencion
                dts.gNombre = NombreTipoRetencion.Text
                dts.gTipoRetencion = TipoRetencion.Text
                dts.gDescripcion = descripcion.Text
                dts.gActivo = ckActivo.Checked
                If iidTipoRetencion = 0 Then 'Guardar
                    iidTipoRetencion = funcTE.insertar(dts)
                    Call cargardatoslistview()
                Else 'Actualizar
    ...
    Private Function Repetido() As Boolean
        For Each item As ListViewItem In lvSecciones.Items
            If item.Index <> indice And item.SubItems(1).Text = NombreTipoRetencion.Text Then
                Return True
            End If
        Next
        Return False
    ...
    Private Sub cargardatoslistview()
        Try
            Dim lista As List(Of DatosTipoRetencion) = funcTE.Mostrar(False)

            Dim dts As DatosTipoRetencion
            lvSecciones.Items.Clear()

            For Each dts In lista
                With lvSecciones.Items.Add(dts.gidTipoRetencion)
                    .SubItems.Add(dts.gNombre)
                    .SubItems.Add(dts.gTipoRetencion & "%")
    ...
    Private Sub ActualizarLinealv(ByVal dts As DatosTipoRetencion)
        If indice <> -1 Then
            With lvSecciones.Items(indice)
                .SubItems(1).Text = dts.gNombre
                .SubItems(2).Text = dts.gTipoRetencion & "%"
    ...
    Private Sub lvSecciones_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lvSecciones.Click, lvSecciones.SelectedIndexChanged
        ...
        If lvSecciones.SelectedItems.Count > 0 Then
            ...
            iidTipoRetencion = lvSecciones.Items(indice).Text
            Try
                Dim dts As DatosTipoRetencion = funcTE.Mostrar1(iidTipoRetencion)
                NombreTipoRetencion.Text = dts.gNombre
                TipoRetencion.Text = dts.gTipoRetencion
