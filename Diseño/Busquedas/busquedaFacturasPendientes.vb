Public Class busquedaFacturasPendientes
    ...
    Private Sub busquedaCliente_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        EstadoDevuelto = funcES.EstadoFactura("DEVUELTA")
        EstadoParcial = funcES.EstadoFactura("PARCIAL")
        EstadoPendiente = funcES.EstadoFactura("PENDIENTE")
...
#Region "PROCEDIMIENTOS Y FUNCIONES"
    ...
    Private Sub busqueda()
        ...
        If txNumFactura.Text <> "" Then
            ...
            sBusqueda = sBusqueda & " DOC.numFactura like '" & Replace(txNumFactura.Text, "'", "''") & "%' "
        End If
        If cbCliente.Text <> "" Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " Clientes.NombreFiscal like '%" & Replace(cbCliente.Text, "'", "''") & "%' "
        End If
        ...
        If cambioFechas Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " DOC.numFactura in(select distinct numFactura from Vencimientos where CONVERT(datetime,CONVERT(Char(10), Vencimiento,103))  >= '" & dtpDesde.Value.Date & "' AND  CONVERT(datetime,CONVERT(Char(10), Vencimiento,103))  <= '" & dtpHasta.Value.Date & "') "
        End If

        If IsNumeric(cbAño.Text) Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " DOC.numFactura in(select  numFactura from Vencimientos where Year(Vencimiento)= " & cbAño.Text & ") "
        End If
        If cbMes.SelectedIndex <> -1 Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " DOC.numFactura in(select  numFactura from Vencimientos where Month(Vencimiento)= " & cbMes.SelectedIndex + 1 & ") "
        End If

        If cbTrimestre.SelectedIndex <> -1 Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " DOC.numFactura in(select  numFactura from Vencimientos where Month(Vencimiento)in "
        ...
        If ckVerSoloCaducados.Checked Then
            sBusqueda = sBusqueda & IIf(sBusqueda = "", "", " AND ")
            sBusqueda = sBusqueda & " ( select COUNT(idVencimiento) from Vencimientos where numFactura = DOC.NumFactura and  Vencimiento < GETdate() and (Cobrado = 0 or Devuelto = 1))>0"
        End If
        Call ActualizarLV()
    ...
    Private Sub ActualizarLV()
        Try
            ...
            Dim lista As List(Of DatosFactura) = funcOF.Busqueda(sBusqueda, Orden, False)
    ...
    Private Sub NuevaLineaLV(ByVal dts As DatosFactura)
        With lvDocumentos.Items.Add(dts.gnumFactura)
            ...
            .SubItems.Add(If(dts.gPrimerVencimientoNoCobrado = CDate("1-1-1900"), "", CStr(dts.gPrimerVencimientoNoCobrado)))
            ...
            End Select
            If funcVE.VencimientosCaducados(dts.gnumFactura) > 0 Then .ForeColor = Color.Red
    ...
    Private Sub ActualizarLineaLV(ByVal inumFactura As Integer)
        Dim dts As DatosFactura = funcOF.Mostrar1(inumFactura)
        With lvDocumentos.Items(indice)
            .SubItems(1).Text = dts.gCliente
            ...
            .SubItems(6).Text = If(dts.gPrimerVencimientoNoCobrado = CDate("1-1-1900"), "", CStr(dts.gPrimerVencimientoNoCobrado))
            Select Case dts.gidEstado
                Case EstadoDevuelto.gidEstado
                    .ForeColor = Color.Red
                Case EstadoParcial.gidEstado
...
#Region "BOTONES GENERALES"
    ...
    Private Sub bListado_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bListado.Click
        Dim GG As New InformeListadoFacturasClientesPtes
...
#Region "EVENTOS"
    ...
    Private Sub lvDocumentos_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lvDocumentos.DoubleClick
        If lvDocumentos.SelectedItems.Count > 0 Then
            ...
            Else
                ...
                Call ActualizarLineaLV(GP.pnumFactura)
    ...
    cbCliente.SelectedIndexChanged, cbEstado.SelectedIndexChanged, cbAño.TextChanged, cbMes.SelectedIndexChanged, cbTrimestre.SelectedIndexChanged
    ...
    Private Sub lvDocumentos_ColumnClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ColumnClickEventArgs) Handles lvDocumentos.ColumnClick
        ...
        Select Case e.Column
            ...
            Case 5
                Orden = "Pendiente"
            Case 6
                Orden = "PrimerVencimientoNoCobrado"
        End Select
        columna = e.Column
        If Orden <> "" Then
    ...
    Private Sub cbCliente_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbResponsable.TextChanged, cbCliente.TextChanged, cbEstado.TextChanged
        Call busqueda()
    End Sub
...
#End Region
    ...
    Private Sub txNumFactura_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txNumFactura.TextChanged
        Call busqueda()
    End Sub
End Class