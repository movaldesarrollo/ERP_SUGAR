...
Public Class GestionProforma
    ...
    Private funcCO As New FuncionesConceptosProformas
    ...
    Private dtsOF As DatosProforma
    Private dtsCL As datoscliente
    Private dtsFA As datosfacturacion
    Private dtsCO As DatosConceptoProforma
    Private dtsAR As DatosArticulo
    Private listaC As List(Of DatosConceptoProforma)
    ...
    Public Property pnumProforma() As Integer
        Get
            Return numDoc.Text
        End Get
    ...
    Property pProformas() As List(Of Integer)
        Get
            Return Proformas
        End Get
    ...
    Private Sub GestionProforma_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        semaforo = True
        iidIdioma = 1
        ...
        Else
            Me.Text = "EDITAR PROFORMA"
            gbConceptos.Enabled = True
            If Proformas Is Nothing Then
                bSubir.Visible = False
                bBajar.Visible = False
            Else
    ...
    Private Sub GestionProforma_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If cambios Then
...
#Region "INICIALIZACIÓN"
    ...
    Private Sub Nuevo()
        ...
        dtsOF = New DatosProforma
        gbConceptos.Enabled = False
        numDoc.Text = funcMA.leernumProforma(Now.Year) + 1
        If numDoc.Text = 0 Then
            funcMA.NuevoAño()
            numDoc.Text = funcMA.leernumProforma(Now.Year) + 1
            If numDoc.Text = 0 Then
        ...
        End If
        G_AGeneral = "G"
        Dim dtsES As DatosEstado = funcES.EstadoCabecera("Proforma")
...
#Region "CARGAR DATOS"
    ...
    Private Sub CargarProforma()
        Try
            ...
            If dtsES.gCabecera Or dtsES.gEspera Then
                Call introducirClientes()
                ClienteSoloLectura = False
                bBuscarCliente.Enabled = True
            Else
                'Sólo carga el cliente en cuestión (porque no se va a poder cambiar)
                cbCliente.Items.Clear()
                cbCodCliente.Items.Clear()
            ...
            If dtsES.gCabecera Then
                cbEstado.Items.Add(funcES.EstadoCabecera("Proforma"))
                cbEstado.Items.Add(funcES.EstadoAnulado("Proforma"))
            End If
            If dtsES.gAnulado Then
                cbEstado.Items.Add(funcES.EstadoEnCurso("Proforma"))
                cbEstado.Items.Add(funcES.EstadoAnulado("Proforma"))
            End If
            ...
            If dtsES.gTraspasado Then
                cbEstado.Items.Add(funcES.EstadoTraspasado("Proforma"))
            End If
    ...
    Private Sub CargarConceptos()
        ...
        lvConceptos.Items.Clear()
        For Each dts As DatosConceptoProforma In listaC
            nuevaLineaLV(dts)
        Next
        cbArticulo.Focus()
    End Sub
    ...
    Private Sub CargarDirecciones()
        If cbCliente.SelectedIndex <> -1 Then
            ...
            cbDireccion.Items.Clear()
            'Dim lista As List(Of datosUbicacion) = funcUB.mostrarCli(cbCliente.SelectedItem.itemdata, True, 1, 1, 0, 0, True, 0)
            Dim lista As List(Of datosUbicacion) = funcUB.mostrarCliProforma(cbCliente.SelectedItem.itemdata, True)
...
#Region "PROCEDIMIENTOS Y FUNCIONES"
    ...
    Private Sub nuevaLineaLV(ByVal dts As DatosConceptoProforma)
    ...
    Private Sub ActualizarLineaLV(ByVal dts As DatosConceptoProforma)
    ...
    Private Function GuardarGeneral()
        Try
            ...
            End If
            If validar Then
                dtsOF.gnumProforma = numDoc.Text
                ...
                If G_AGeneral = "G" Then
                    ...
                    dtsOF.gnumProforma = numDoc.Text
                    G_AGeneral = "A"
                    MsgBox("Creada la Proforma " & numDoc.Text & ". Ya puede introducir los conceptos.")
                    gbConceptos.Enabled = True
                    cbEstado.Items.Add(funcES.EstadoAnulado("Proforma"))
                ...
                Else
                    ...
                    Call Recalcular()
                    'MsgBox("Proforma actualizada correctamente")
                End If
                cambios = False
                ep1.Clear()
                ep2.Clear()
        ...
        Try
            ...
            If cbArticulo.Text <> "" Then
                ...
                If indice = -1 Then
                    ...
                    Dim estado As DatosEstado = funcES.EstadoEspera("C.PROFORMA")
                    ...
                    If lvConceptos.Items.Count > 0 Then 'Si hay items, modificar los estados de la proforma
                        Me.Text = "EDITAR PROFORMA"
                        'cbEstado.Items.Clear()
                        'cbEstado.Items.Add(funcES.EstadoAnulado("Proforma"))
                        cbEstado.SelectedIndex = -1
                        cbEstado.Text = ""
                        Dim x As Integer = cbEstado.FindString(funcES.EstadoCabecera("PROFORMA").gEstado)
                        If x <> -1 Then cbEstado.Items.RemoveAt(x) 'Eliminar el estado cabecera

                        Dim dtsES As DatosEstado = funcES.EstadoEnCurso("Proforma")
...
#Region "BOTONES GENERALES"
    ...
    Private Sub bBajar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bBajar.Click
        If indiceL < Proformas.Count - 1 Then
            If cambios Then
                If MsgBox("Se perderán los datos no guardados", MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                    Call InicializarGeneral()
                    indiceL = indiceL + 1
    ...
    Private Sub bBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bBorrar.Click
        ...
        If indice = -1 Then
            'Borrar Proforma
            If G_AGeneral = "G" Then  'Si aún no hemos guardado la Proforma, es como pulsar nuevo
                If cambios Then
                ...
                End If
            Else
                If MsgBox("No es posible borrar una Proforma existente. ¿Desea anularla?", MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                    Dim dtsES As DatosEstado = funcES.EstadoAnulado("Proforma")
                    cbEstado.Text = dtsES.ToString
                    funcOF.actualizaEstado(numDoc.Text, dtsES.gidEstado)
                    bGuardar.Enabled = False
                    bBorrar.Enabled = False
    ...
    Private Sub bNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bNuevo.Click
        ...
        Else
            If cambios Then
                If MsgBox("Se perderán los los datos introducidos y se creará una nueva proforma. ¿Continuar?", MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                    Call InicializarGeneral()
                    Call introducirClientes()
                    Call Nuevo()
                End If
            Else
                If MsgBox("Se creará una nueva proforma. ¿Continuar?", MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                    Call InicializarGeneral()
                    Call introducirClientes()
                    Call Nuevo()
                End If
    ...
    Private Sub bTraspasar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bTraspasar.Click
        ...
        If ckSeleccion.Visible And N > 0 Then
            ...
            End If
        Else
            If MsgBox("Seleccione los conceptos que se han de convertir en Pedido ", MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                ckSeleccion.Visible = True
                lvConceptos.CheckBoxes = True
            End If
        End If
    ...
    Private Sub bPDF_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bPDF.Click
        ...
        ElseIf lvConceptos.Items.Count = 0 Then
            MsgBox("No se han introducido conceptos")
        Else
            Dim fichero As String = "Proforma SV " & Trim(numDoc.Text) & " " & Microsoft.VisualBasic.Left(cbCliente.Text, 40) & ".pdf"
            Dim sfd As New SaveFileDialog
            sfd.FileName = fichero
            sfd.ShowDialog()
            InformeProforma.GeneraPDF(numDoc.Text, iidIdioma, sfd.FileName, "")
    ...
    Private Sub beMail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles beMail.Click
        ...
        Else
            ...
            InformeProforma.GeneraPDF(numDoc.Text, iidIdioma, fichero, camino)
            ...
            texto = texto & "<br/><br/> Adjuntamos la Proforma " & numDoc.Text & ". "
            'Se envía un correo outlook a la dirección de la ubicación
            CorreoOutlook("PROFORMA CLIENTE", funcPE.campoCorreo(iidUsuario), Destinatario, camino & fichero, cbContacto.Text, numDoc.Text, dtpFecha.Value.Date, dtpFechaPrevista.Value.Date, iidIdioma)
            'CorreoOutlook("Proforma SUGAR VALLEY", texto, funcPE.campoCorreo(iidUsuario), Destinatario, camino & fichero)
        End If
    End Sub
...
#Region "EVENTOS"
    ...
    Private Sub ProntoPago_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Nota.TextChanged, Observaciones.TextChanged, ProntoPago.TextChanged, dtpFecha.ValueChanged, dtpFechaPrevista.ValueChanged, RefCliente.TextChanged, cbRetencion.SelectedIndexChanged, cbTipoIVA.SelectedIndexChanged, cbCuenta.SelectedIndexChanged, cbContacto.SelectedIndexChanged, cbSolicitadoVia.SelectedIndexChanged, PrecioTransporte.TextChanged, PrecioTransporte.TextChanged, ckRecargoEquivalencia.CheckedChanged
        cambios = True
    End Sub
    ...
    Private Sub cbDireccion_SelectionChangeCommitted(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbDireccion.SelectionChangeCommitted
        'iidIdioma = funcUB.campoIdioma(dtsOF.gidUbicacion)
        Dim dtsUB As datosUbicacion = funcUB.mostrar1(cbDireccion.SelectedItem.itemdata)
        PrecioTransporte.Enabled = False
