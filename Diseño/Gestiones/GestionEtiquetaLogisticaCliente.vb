...
#Region "EVENTOS"
    ...
    Private Function pdfEquals() As Boolean
        ...
        End If
        If result = True Then
            MsgBox("El PDF que quieres substituir es el que se está utilizando actualmente.")
        End If
        Return result
    End Function
...
#Region "FUNCIONES"
    ...
    Private Sub LoadDataCustomerTag(data As DatosEtiquetaLogisticaCliente)
        With data
            ...
            Catch ex As ArgumentException
                MsgBox("LoadDataCustomerTag" & ex.ToString)
            End Try
        End With
    ...
    Private Function existsPDF() As String
        ...
        If tbPathPDF.Text.Trim <> "" Then
            pathPDF = clonePDF()
            If pathPDF <> "" Then
                MsgBox("PDF clonado correctamente.")
            Else
                MsgBox("No se ha podido clonar el PDF.")
            End If
        Else
            Return pathPDF
        End If
    ...
    Friend Function clonePDF() As String
        ...
        Catch ex As Exception
            MsgBox("ERROR AL COPIAR EL PDF." & ex.ToString)
            result = ""
        End Try
        Return result
    End Function
    ...
    Private Sub UpdateCustomerTag()
        ...
        Try
            ...
            updated = funGEC.UpdateCustomerTag(imgClient, idEtiquetaCliente, widthImgCliente, heightImgCliente, rutaPDF)
            'If Not tbPathPDF.Text.Trim.Equals("") And pdfModificado Then
            '    clonePDF()
            'End If
            'CleanPDFFromPath()
        Catch ex As Exception
            MsgBox("UpdateCustomerTag  " & ex.ToString)
        End Try
        If updated Then
